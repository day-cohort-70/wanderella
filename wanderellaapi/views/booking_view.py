...
class BookingView(ViewSet):
    ...
    def create(self, request):
        ...
        if hotel is None or flight is None or car is None or price is None or book_date is None:
            return Response({'message': 'You must provide all required keys in the JSON body'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            new_booking = Booking()
            new_booking.customer = UserProfile(user=request.auth.user)
            ...
            return Response({'message': 'You provided an invalid `id` for the hotel, or flight, or car'}, status=status.HTTP_400_BAD_REQUEST)
